...
Use `api.stream` for chunked delivery when the full payload should not be buffered first:
...
Add explicit headers with `api.stream_with_headers`:

```lua
function api.export.json.get(ctx)
    local i = 0
    ...
    return api.stream_with_headers(200, "application/json", {
        ...
        if i == 2 then return '{"n":1}' end
        if i == 3 then return ',{"n":2}]' end
        return nil
    end)
end
```

The producer returns the next chunk as a string, or `nil` to end the stream.

## Server-Sent Events

Use `api.sse` for browser-friendly one-way event streams:
...
| `api.stream` | custom | custom | `api.stream(200, "text/plain", fn)` |
| `api.stream_with_headers` | custom | custom | `api.stream_with_headers(200, "application/json", headers, fn)` |
| `api.sse` | 200 | `text/event-stream` | `api.sse(fn, 1500)` |
| Plain table | 200 | `application/json` | `{ message = "..." }` |

## Next Steps

- [Configuration](/docs/server/configuration/) - Configure server options
- [Route Patterns](/docs/server/route-patterns/) - Learn about routing
- [Streaming](/docs/http-and-realtime/streaming/) - Long-lived responses, SSE, proxy notes
