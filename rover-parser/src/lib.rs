...
mod tests {
    ...
    fn should_parse_rest_api_basic() {
        ...
        assert!(route.request.path_params[0].used, "id param should be marked as used");
        ...
        assert!(route.request.path_params[0].used, "id param should be marked as used");
        assert!(route.request.path_params[1].used, "postId param should be marked as used");
        ...
        assert!(route.request.path_params[0].used, "name param should be marked as used");
    }
    ...
    fn should_parse_rest_api_auth() {
        ...
        assert_eq!(hello_route.request.headers[0].schema.guard_type, crate::analyzer::GuardType::String);
        assert!(!hello_route.request.headers[0].schema.required);

        // Check status code parsing
        assert_eq!(hello_route.responses.len(), 2, "Should have 2 responses (200 and 401)");
        let success_response = hello_route.responses.iter().find(|r| r.status == 200).unwrap();
        let error_response = hello_route.responses.iter().find(|r| r.status == 401).unwrap();
    ...
    fn should_parse_context_requests() {
        ...
        println!("GET route query params: {}", echo_get.request.query_params.len());
    ...
    fn should_parse_validation_guard() {
        ...
        assert!(basic_route.request.body_schema.is_some(), "Should have body schema");
        ...
        assert!(body_schema.schema["required"].as_array().unwrap().contains(&"name".into()));
        assert!(body_schema.schema["required"].as_array().unwrap().contains(&"email".into()));
    }
    ...
    fn should_parse_validation_reference() {
        ...
        assert_eq!(body_schema.schema["properties"]["tags"]["items"]["type"], "string");

        // Check nested objects route
        let nested_route = server.routes.iter().find(|r| r.path == "/nested_objects").unwrap();
        ...
        assert_eq!(body_schema.schema["properties"]["user"]["properties"]["name"]["type"], "string");
        assert_eq!(body_schema.schema["properties"]["user"]["properties"]["profile"]["type"], "object");
    }
...
return api
        ...
        assert!(!model.errors.is_empty(), "Should have errors about non-existent params");
        let param_error = model.errors.iter().find(|e| e.message.contains("nonexistent")).unwrap();
